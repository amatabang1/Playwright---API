...
- Name: SBBankList.spec.ts >> Sandbox - Bank List API Test Suite >> TS-EB-SB-PRODUCTS-0092: Send Funds via Local Fund Transfer v1.0 _Get Supported Banks and Wallets API_Verify when the “Transfer Network” (channel) is set to specific and API returns only Financial Institutions that support both transfers
...
> 33 |     const buffer = XLSX.write(workbook, {
  ...
  71 |     ? value.slice(0, MAX_EXCEL_CELL_LENGTH - 25) + "\n[TRUNCATED]"
  72 |     : value;
  73 | }
```